...
export default function TasaPage() {
  ...
  const fechaActualizada = configuracion?.tasa_actualizada_en
    ? new Date(configuracion.tasa_actualizada_en).toLocaleDateString('es-VE', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })
    : null;
    ...
    <div>
      ...
      <div className="p-4 space-y-4">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
          ...
          {tasa > 0 ? (
            <>
              <p className="text-4xl font-bold text-gray-900">{tasa.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              <p className="text-gray-400 text-sm mt-1">Bs por 1 USD</p>
            </>
          ) : (
            <p className="text-2xl text-gray-400 font-medium">No configurada</p>
          )}
          {fechaActualizada && <p className="text-gray-400 text-xs mt-3">Actualizada: {fechaActualizada}</p>}
        </div>
            ...
            <div className="space-y-1 text-sm">
              {[1, 5, 10, 20, 50, 100].map(usd => (
                <div key={usd} className="flex justify-between">
                  ...
                  <span className="font-semibold text-gray-800">Bs {(usd * tasa).toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          ...
          {!isOnline && (
            <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">Sin conexión — la tasa no puede guardarse</div>
          )}
          <div className="flex gap-2">
            ...
            />
            <button onClick={handleGuardar} disabled={guardando || !input || !isOnline}
